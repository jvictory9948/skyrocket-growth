  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-primary p-4 flex items-center gap-3">
        ...
        <div>
          <h3 className="text-white font-heading font-semibold">How to Order</h3>
          <p className="text-white/80 text-xs">Quick video guide</p>
        </div>
      </div>
        ...
        {!isPlaying ? (
          ...
          >
            ...
            <motion.div
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              className="relative z-10 w-16 h-16 rounded-full bg-primary flex items-center justify-center shadow-lg purple-glow group-hover:scale-110 transition-transform"
            >
              <Play className="h-6 w-6 text-white ml-0.5" fill="currentColor" />
            </motion.div>
            
            {/* Text */}
            <span className="absolute bottom-4 left-1/2 -translate-x-1/2 text-xs text-muted-foreground font-medium">
              Click to play
            </span>
          </button>
        ) : (
